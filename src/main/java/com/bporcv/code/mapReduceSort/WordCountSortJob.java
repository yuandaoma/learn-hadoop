package com.bporcv.code.mapReduceSort;
 ...
 */
public class WordCountSortJob extends Configured implements Tool {

    public static final String HADOOP_URI = "hdfs://node1:8020";
    public static final String PATH = "/wordCountSort";
    ...
    public int run(String[] args) throws Exception {
        try {
            // 创建一个任务
            Job job = Job.getInstance(super.getConf(), WordCountSortJob.class.getSimpleName());
            // 打包到集群上运行，必须要指定运行的主类
            ...
            // 设置分区
//            job.setSortComparatorClass(WordCountSortComparator.class);
    ...
    public static void main(String[] args) {
        try {
            ...
            Tool tool = new WordCountSortJob();
    ...
    private static void generatePartitionerData() {
        try {
            ...
            FSDataOutputStream os = fileSystem.create(new Path(PATH + File.separator+ DATA_FILE));
            os.write(text.getBytes());
            os.flush();
            fileSystem.close();
            System.out.println("排序文件创建成功");
